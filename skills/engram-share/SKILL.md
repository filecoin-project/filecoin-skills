...
**First run (nothing configured):** stop and instruct the user to create `~/.filecoin-pin.env` themselves containing `PRIVATE_KEY=0x...` for a funded wallet (FIL for gas, USDFC for storage), then `chmod 600` it. Never offer to write the key file, and never ask for the key in the conversation. (Session keys are a real CLI capability today — `filecoin-pin session create`, or `session generate` + `session authorize` for the two-party flow — but this skill does not yet drive them; see `references/filecoin-pin-cli.md` under Authentication. Note there is no browser-pairing flow in the released CLI: `session create --console` and `session import` do not exist in v2.0.1.)
