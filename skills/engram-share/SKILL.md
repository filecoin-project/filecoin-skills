...
**FWSS storage only:** `--egress-provider none` on every add. FilBeam CDN egress locks an extra 1 USDFC per new data set and is enabled ONLY when the user explicitly asks for CDN egress — same for any future add-on.
