...
**Session mode: data operations only.** A session key can create data sets and add pieces; it cannot move money. The authorizable scopes are exactly `createDataSet`, `addPieces`, `schedulePieceRemovals` and `terminateService` — there is no payments scope — so `payments setup`, `payments fund`, `payments deposit` and `add --auto-fund` all need the owner wallet. In session mode: step 3 runs `payments status` read-only and never `payments setup --auto`; step 5 omits `--auto-fund` and `--max-balance`; and a funding shortfall is not yours to fix — stop, report the shortfall, and tell the user to fund from the owner wallet (the Filecoin Pay console is the easiest route), then re-run. This matches upstream's own design, not just this skill's reading of the scope list: filecoin-pin#702 states plainly that "session keys cannot deposit, so there is no auto-fund on this path", and adds a pre-upload funds preflight that blocks the add and prints a pre-filled console funding link instead. Still verify against the installed CLI the first time you hit it — the payments commands do accept session flags, and v2 gates scopes per command, so expect a clear refusal rather than silent success.
